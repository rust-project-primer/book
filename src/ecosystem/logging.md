...
Asynchronous code often has a lot of *concurrency*, meaning that multiple
...
Tracing improves on this by being *scoped*, this means that tracing events are
...
takes each task to complete (which corresponds to *latency*, if each task
handles a request).
