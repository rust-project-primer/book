...
const response = await fetch("https://crates.io/api/v1/crates?page=1&per_page=100&sort=recent-downloads&keyword=embedded-hal");
const crates = await response.json();
const data = crates["crates"].map(crate => ({"id": crate["id"], "value": crate["downloads"]}));
